...
본문에 `## What was done`과 `## Open and next` 섹션이 반드시 있어야 한다(lint 검사 대상).
`## Decisions and why`와 `## Cautions`는 생성기가 함께 넣지만 강제하지 않는다.
**섹션 제목은 영어 정본이다** — 생성기가 찍고 lint가 그대로 검사한다(ADR-003).
본문 내용은 어떤 언어로 써도 된다.
